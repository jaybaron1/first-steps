...
const LiveVisitorMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchActiveVisitors();
    const interval = setInterval(fetchActiveVisitors, 30000); // Refresh every 30s
  ...
  if (loading) {
    return (
      <div className="h-[200px] flex items-center justify-center">
        <div className="w-6 h-6 border-2 border-[#B8956C] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <path
          d="M120,120 Q160,100 200,110 T260,100 T300,120 T280,160 T220,180 T160,160 Z"
          fill="currentColor"
          className="text-[hsl(var(--admin-text-subtle))]"
        />
        <path
          d="M320,80 Q380,60 440,70 T520,80 T560,100 T580,140 T560,180 T500,200 T440,190 T380,160 T340,120 Z"
          fill="currentColor"
          className="text-[hsl(var(--admin-text-subtle))]"
        />
        <path
          d="M560,100 Q620,80 680,90 T740,110 T760,150 T740,190 T680,200 T620,180 T580,140 Z"
          fill="currentColor"
          className="text-[hsl(var(--admin-text-subtle))]"
        />
        <path
          ...
          return (
            ...
            >
              <div className="relative">
                ...
                <div className="relative w-3 h-3 rounded-full bg-emerald-500 border-2 border-[hsl(var(--admin-bg))]" />
                
                {/* Tooltip */}
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                  <div className="bg-[hsl(var(--admin-bg-card))] border border-[hsl(var(--admin-border))] rounded px-2 py-1 text-xs whitespace-nowrap">
      ...
      {/* Legend */}
      <div className="absolute bottom-2 right-2 flex items-center gap-2 text-xs text-[hsl(var(--admin-text-subtle))]">
        <div className="w-2 h-2 rounded-full bg-emerald-500" />
        <span>{visitors.length} active visitor{visitors.length !== 1 ? 's' : ''}</span>
      </div>

      {visitors.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-[hsl(var(--admin-text-subtle))]">
            <MapPin className="w-8 h-8 mx-auto mb-2 opacity-30" />
            <p className="text-sm">No active visitors with location data</p>
          </div>
        </div>
      )}
    </div>
