...
const PartnersMyPage: React.FC = () => {
  ...
  const { data: clicks = [] } = useQuery({
    queryKey: ["partner-self", "clicks", partnerId],
    enabled: !!partnerId,
    queryFn: async () => {
  ...
  const { data: clients = [] } = useQuery({
    queryKey: ["partner-self", "clients", partnerId],
    enabled: !!partnerId,
    queryFn: async () => {
      const { data } = await supabase
  ...
  const { data: events = [] } = useQuery({
    queryKey: ["partner-self", "events", partnerId],
    enabled: !!partnerId,
    queryFn: async () => {
      const { data } = await supabase
        .from("commercial_events")
        .select(
          "id, event_type, amount_charged, commission_amount, payment_status, event_date, client:partner_clients!inner(partner_id, client_name)",
        )
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Your referral activity at a glance.</p>
      </div>
